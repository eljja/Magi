...
export async function prepareSelfImprovementSafety(input: {
  directory: string
  title: string
  prompt: string
  ...
  const branch =
    input.config.safety.branchSelfImprovement && inside.code === 0 && clean
      ? branchName(input.config.safety.branchPrefix, input.title, runID)
      : undefined
  ...
  const reportPath = input.config.safety.writeRunReport
    ? await writeRunReport(input.directory, {
        ...prepared,
        title: input.title,
        prompt: input.prompt,
        createdAt: Date.now(),
...
export function safetyPrompt(input: { prompt: string; safety?: MagiSafetyPreparation }) {
...
export async function writeMagiRunDecision(input: {
  directory: string
...
async function writeRunReport(directory: string, report: Record<string, unknown>) {
