...
**2026-09-20 운영 방향:** 기본 지속 모드는 완료 표결 없이 진행 증거를 다음 회의로 넘기고, 4시간 가동 또는 세 인격의 주요 변화 합의 때 보고합니다. [이번 변경·설치·검증 범위](CONTINUOUS-OPERATION.ko.md)를 먼저 확인하세요. 아래 CI와 완료 표결 검증은 해당 커밋 당시 기록이며 이번 수정의 새 CI 통과로 해석하면 안 됩니다. npm 공개 버전은 여전히 0.1.0이며 소스 0.1.1은 아직 게시되지 않았습니다.
