...
**최신 전체 평가:** [최초 설치·CLI·GUI 실측 결과](END-TO-END-AUDIT.ko.md)에 실제 무료 모델에서 발견한 추가 결함과 수정, 각 화면의 통과 범위와 미통과 항목을 정리했습니다. 공개 npm 0.1.1과 이후 소스 수정은 별개입니다.

**2026-09-20 운영 방향:** 기본 지속 모드는 완료 표결 없이 진행 증거를 다음 회의로 넘기고, 4시간 가동 또는 세 인격의 주요 변화 합의 때 보고합니다. [이번 변경·설치·검증 범위](CONTINUOUS-OPERATION.ko.md)를 먼저 확인하세요. **npm 0.1.1 공개와 배포 파일 일치를 확인했습니다.** [최신 게시 기록](#publication-011)과 아래 과거 0.1.0 감사 기록을 구분합니다.

<a id="publication-011"></a>

## 0.1.1 공개 확인 — 2026-09-20

사용자가 Chrome에서 npm 추가 인증을 마친 뒤 실제 `npm publish`가 종료 코드 0과 `+ oh-my-magi@0.1.1`을 반환했습니다. npm의 비동기 처리 직후에는 0.1.0만 조회됐으나, 이후 공개 레지스트리에서 정확한 버전 `0.1.1`과 `latest: 0.1.1`을 확인했습니다. 인증 완료나 게시 접수만으로 공개 성공을 판정하지 않았습니다.

- 공개 패키지: [oh-my-magi 0.1.1](https://www.npmjs.com/package/oh-my-magi/v/0.1.1)
- 레지스트리 SHA-1: `b89ea2f4dcf90327ce312f203387d2635d0fa18a`
- 직접 내려받은 파일의 SHA-256: `4467760ce99f6d11734d67f7e593044c414e9909534897d5876b440225ca398b`
- 검토한 후보와 내려받은 파일이 일치하며 실제 의존성은 OmO `4.19.4`, OpenCode SDK/plugin `1.18.31`입니다.
- 구현 기준은 [`d14151c15215`](https://github.com/eljja/Magi/commit/d14151c1521531b76ce10a037554b611ba6f9374)이며, 해당 구현의 [6개 플랫폼·버전 조합과 코어 CI](https://github.com/eljja/Magi/actions/runs/35451257603)가 통과했습니다.
- 공개 후 별도의 빈 설정·캐시와 Git 없는 PATH에서 실제 OpenCode 1.18.31의 `opencode plugin oh-my-magi@0.1.1 --global`을 실행했습니다. 로컬 경로 대신 npm 패키지 이름으로 설치됐고, OmO 선행 설치 없이 OMM 0.1.1·OmO 4.19.4 및 서버/TUI 등록을 확인했습니다.
- 같은 설치로 재현용 모델 통합 검사가 종료 코드 0으로 통과했습니다. 실제 OmO 위임, 3회 이상 연속 진행, 완료 표결 없는 연결, 주요 변화 보고의 대화 전달, 일반 대화 개입과 명령 기반 중지·재개를 확인했습니다. 증거 폴더는 `magi-smoke-gQaCle`, 로그는 `.tmp/omm-integration/registry-0.1.1-native.log`입니다. 이는 실제 모델의 판단 능력 검증과 구분합니다.

패키지를 검토한 상태 그대로 게시했으므로 압축 파일 내부 README에는 게시 전 안내가 남아 있습니다. 최신 배포 상태와 사용법은 이 저장소의 README를 기준으로 확인하세요. npm 게시 성공은 실제 무료 모델의 장시간 자율개발 성공과 별개이며, [장시간 시험](SOAK-VALIDATION.ko.md)은 별도로 판정합니다.

**2026-09-19 최신 개발:** [세 인격 기반 자율 개발 설계와 수정 내용](AUTONOMY-DESIGN.ko.md)에 분리된 의회·OmO 실행, 중간 회의와 검토 복구, 인격별 기억, 실제 모델 시험 방법을 정리했습니다. 아래는 날짜가 명시된 과거 출시 기록입니다.

**2026-09-19 코드 검증(과거):** 실행 완료도 세 인격이 토론·투표하도록 확장하고, 산출물 제출·실제 파일 증거·검토 중 변경 감지·종료 경쟁 상태를 수정했습니다. 103개 회귀 테스트·484개 검증 조건과 [Linux·macOS·Windows 최소/최신 OpenCode 6개 조합 및 코어 CI](https://github.com/eljja/Magi/actions/runs/35415094451)가 모두 통과했습니다. 코드 기준은 `157f66e2`이며, 실제 무료 모델의 전체 자율개발 성공과 npm 게시 여부는 별도입니다.

**2026-09-18 추가 점검:** 아래는 0.1.0의 기록입니다. 에이전트 선택 자동 시작·Git 비필수 실행·설치 문제를 수정한 최신 내용은 [0.1.1 사용 흐름 재점검](USER-FLOW-AUDIT.ko.md)을 확인하세요.

**실제 LLM 후속 시험:** OpenRouter 무료 모델을 실제 OpenCode에 연결한 [별도 검증 결과](REAL-MODEL-AUDIT.ko.md)를 추가했습니다. 수정판은 실제 코드 수정과 보존된 원본 테스트 3개 통과까지 확인했습니다. 독립 완료 검토·후속 회의의 전체 흐름은 아직 미통과이며 반복 조사·공급자 오류도 남아 있습니다. 아래의 결정적 모델 통합 성공과 구분해야 합니다.
