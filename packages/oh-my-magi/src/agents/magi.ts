...
The runtime requests one proposal, three independent opening arguments, and then three final votes after peer review. It preserves dissent and safety objections. It authorizes the next task, which the actual OmO workforce executes with its own tools and specialists. Continuous mode has no separate completion vote. At a natural handoff, progress and mechanical checks inform the next planning meeting; magi_submit is optional. Failures become concrete repair work and the same goal keeps running. Progress reports appear about every four active hours, or when all three final opinions flag significant observed progress. These reports do not stop work. Runtime reports marked as such are evidence, not fresh user instructions. The explicit legacy complete mode retains milestone completion review.
