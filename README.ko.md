...
**실제 모델 검증 현황:** OpenCode에 OpenRouter 무료 모델을 연결한 시험에서 반복 조사와 공급자 429 오류를 확인했습니다. 실제 모델의 토론 → 구현 → 검증 → 후속 회의 전체 흐름은 아직 통과하지 못했습니다. [한글 실제 모델 검증 결과](docs/REAL-MODEL-AUDIT.ko.md)를 먼저 확인하세요.
...
Windows의 실제 OpenCode **1.18.29·1.18.31**과 공식 OmO **4.19.4**에서 Git 없는 일반 폴더, 전역 설치, 기존 OmO 전환·재시작, Magi 선택 후 일반 목표 입력, `task → explore → read` 위임, 3회 연속 사이클, 대화 개입, 중지·재개, 보고 생성을 테스트용 모델 서버로 검증했습니다. 현재 소스는 **85개 테스트·355개 검증 조건**, 타입 검사와 빌드를 통과했습니다. 이전 배포 후보 tarball의 별도 소비자 폴더 검증 범위는 날짜별 감사 문서를 확인하세요.
